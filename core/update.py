...
class ConvGRU(nn.Module):
    ...
    def forward(self, h, cz, cr, cq, *x_list):
        # self.gru08(net[0], *(inp[0]), motion_features, interp(net[1], net[0]))
        # print(h.shape, cz.shape, cr.shape, len(x_list))
        # torch.Size([1, 128, 64, 128]) torch.Size([1, 128, 64, 128]) torch.Size([1, 128, 64, 128]) 2
        ...
        # print(z.shape, r.shape, q.shape)
        # torch.Size([1, 128, 64, 128]) torch.Size([1, 128, 64, 128]) torch.Size([1, 128, 64, 128])
        h = (1 - z) * h + z * q
        # print((1-z).shape, h.shape)
        # torch.Size([1, 128, 64, 128]) torch.Size([1, 128, 64, 128])
        return h
...
class BasicMotionEncoder(nn.Module):
    ...
    def forward(self, disp, cv, context):
        # print(disp.shape, cv.shape, context.shape)
        # torch.Size([1, 1, 64, 128]) torch.Size([1, 18, 64, 128]) torch.Size([1, 128, 64, 128])
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[], cor_dim=4, disp_dim=3, K_value=3):
        super().__init__()
        self.args = args
        self.encoder = BasicMotionEncoder(args, cor_planes=2*(2*cor_dim+1)*K_value, disp_dim=disp_dim)
