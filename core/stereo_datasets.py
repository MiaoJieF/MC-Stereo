...
class KITTI12(StereoDataset):
    def __init__(self, aug_params=None, root='/data/KITTI_2012', image_set='training'):
        ...
        disp_list = sorted(glob(os.path.join(root, 'training', 'disp_occ/*_10.png'))) if image_set == 'training' else [
                                                                                                                          osp.join(
                                                                                                                              root,
                                                                                                                              'training/disp_occ/000085_10.png')] * len(
            image1_list)
...
class KITTI15(StereoDataset):
    def __init__(self, aug_params=None, root='/data/KITTI_2015', image_set='training'):
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name == 'sceneflow':
            ...
            kitti12 = KITTI12(aug_params)
            logging.info(f"Adding {len(kitti12)} samples from KITTI12")
            kitti15 = KITTI15(aug_params)
            logging.info(f"Adding {len(kitti15)} samples from KITTI15")
            new_dataset = kitti12 + kitti15
        elif dataset_name == 'kitti12':
            new_dataset = KITTI12(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from KITTI12")
        elif dataset_name == 'kitti15':
            new_dataset = KITTI15(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from KITTI15")
