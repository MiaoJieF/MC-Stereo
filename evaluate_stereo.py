...
from mc_stereo import MCStereo, autocast
...
def validate_kitti(model, iters=32, mixed_prec=False):
    ...
    val_dataset = datasets.KITTI15(aug_params, image_set='training')
...
def validate_things(model, iters=32, mixed_prec=False):
    ...
    d1 = 100 * np.mean(out_list)
    f = open('test.txt', 'a')
    f.write("Validation FlyingThings: %f, %f\n" % (epe, d1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_ckpt', help="restore checkpoint",
                        default='ckpt/mc-stereo.pth')
    parser.add_argument('--dataset', help="dataset for evaluation", default='things',
                        choices=["eth3d", "kitti15", "things"] + [f"middlebury_{s}" for s in 'FHQ'])
    parser.add_argument('--mixed_precision', action='store_true', help='use mixed precision')
    parser.add_argument('--valid_iters', type=int, default=0, help='number of flow-field updates during forward pass')
